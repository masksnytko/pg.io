...
class Connection extends Events {
    constructor(options, notifiy, pgType) {
        ...
        this.on('ready', () => {
            for (var el of this.stack) {
                this.stream.write(el);
            }
            this.stack = [];
        });
    ...
    send() {
        this.stream.write(this.wBuf.slice(0, this.offset));
        this.offset = 0;
    }
    save() {
    ...
    R() {
        if (this.len === 12) {
            let str = postgresMd5PasswordHash(this.md5PasswordUser, this.rBuf.slice(9)) + Z;
            this.wString('p');
            this.wInt32BE(Buffer.byteLength(str) + 4);
            this.wString(str);
            this.send();
    ...
    E() {
        this.queue.shift()(this.rBuf.utf8Slice(6, this.len).replace(/\u0000/g, '\n'));
    }
}
